...
def calc_values():
    ...
    x = cursor.fetchall()
    conn.commit()
    cols = [column[1] for column in x]
    df = pd.DataFrame(cols)
    col_list = list(df[0])
    df3 = pd.DataFrame(x)
    yca = yf.Tickers(col_list).history(period="24h")
    df = pd.DataFrame(yca)
    df = df.reset_index()
    ...
    database_df = df3[[1, 2, 3]]
    database_df.set_index(1, inplace=True)
    integrated_df = database_df.join(df2, how='inner')
    usd_try = yf.Ticker("TRY=X").history(period="24h")
    usd_float = usd_try.iloc[0][1]
    integrated_df["tare_value"] = integrated_df[2] * integrated_df[0]
    integrated_df['total_value'] = integrated_df.apply(lambda x: x['tare_value'] * usd_float if x.name == 'GC=F' else x['tare_value'], axis=1)
    return round(integrated_df['total_value'].sum(),2)
...
if st.session_state["authentication_status"] is True:
    ...
    totalcost = st.sidebar.button('Total Cost',type='primary')
    if totalcost:
    ...
    totalvalue=st.sidebar.button('Total Value of Assets')
    if totalvalue:
        result=calc_values()
        st.write('Total Value of Assets: %s' % result)

    wholeassets= st.sidebar.button('All the Assets')
    if wholeassets:
        result2 = all_assets()
